...
def clean_extractions(df,latest_statement_index):
    ...
    for column in df.columns[idx_extraction:(idx_extraction+6)]:
        df.loc[latest_statement_index:,column] = df.loc[latest_statement_index:,column].astype(str)
        df.loc[latest_statement_index:,column] = df.loc[latest_statement_index:,column].str.replace("[", "",regex=False)
        df.loc[latest_statement_index:,column] = df.loc[latest_statement_index:,column].str.replace("]", "",regex=False)
        df.loc[latest_statement_index:,column] = df.loc[latest_statement_index:,column].str.replace('[]', "",regex=False)
    return df
...
def get_scores(df=extractions_df,
    ...
    for i in range(0, len(fields_list)):
        ...
        df.loc[latest_statement_index:, fields_list[i]] = df.iloc[latest_statement_index:, column_index].apply(lambda x: analyze_gpt35(x, system_message))
    ...
    for column in df.columns[(idx_extraction+6):]:
        ...
        df.loc[latest_statement_index:, column] = df.loc[latest_statement_index:, column].str.replace("[", "",regex=False)
        df.loc[latest_statement_index:, column] = df.loc[latest_statement_index:, column].str.replace("]", "",regex=False)
        df.loc[latest_statement_index:, column] = df.loc[latest_statement_index:, column].str.replace("''", "0",regex=False)
        df.loc[latest_statement_index:, column] = df.loc[latest_statement_index:, column].apply(pd.to_numeric, errors='coerce')
    ...
    df.iloc[latest_statement_index:, positive_extractions] = df.iloc[latest_statement_index:, positive_extractions].abs()
    df.iloc[latest_statement_index:, negative_extractions] = -df.iloc[latest_statement_index:, negative_extractions].abs()
    ...
    if write_data:
        ...
        # new data
        new_data = pd.DataFrame(df.iloc[latest_statement_index:,])
